...
export default class TableSelection {
  ...
  correctBoundary() {
    ...
    tableCells.forEach(tableCell => {
      let { x, x1, y, y1, width, height } = getRelativeRect(
      ...
      if (isCellIntersected) {
        this.boundary = computeBoundaryFromRects(this.boundary, { x, x1, y, y1, width, height });
      }
    })
  }
  ...
  setSelection(startTd, endTd, table) {
    table = table ?? this;
    table.boundary = computeBoundaryFromRects(
      getRelativeRect(startTd, table.quill.root.parentNode),
      getRelativeRect(endTd, table.quill.root.parentNode)
    );
    table.correctBoundary();
    table.selectedTds = this.computeSelectedTds();
    table.repositionHelpLines();
  }
...
function computeBoundaryFromRects(startRect, endRect) {
  let x = Math.min(startRect.x, endRect.x);
  let x1 = Math.max(startRect.x1, endRect.x1);
  let y = Math.min(startRect.y, endRect.y);
  let y1 = Math.max(startRect.y1, endRect.y1);
  let width = x1 - x;
  let height = y1 - y;
